...
mod tests {
    ...
    fn test_no_insecure_directories() {
        ...
        assert!(!config_content.contains("\"/tmp\""), "/tmp found in config allowed directories");
        assert!(!logging_content.contains("\"/tmp\""), "/tmp found in logging allowed directories");
        
        // Verify secure directories are present
        assert!(config_content.contains("\"/var/log\""), "Missing /var/log in config");
        assert!(logging_content.contains("\"/var/log\""), "Missing /var/log in logging");
    }
}

...
mod tests {
    ...
    fn test_unsafe_libc_error_handling() {
        ...
        assert!(uid_content.contains("Invalid UID returned"), 
        ...
        assert!(limits_content.contains("errno"), 
                "Missing errno reporting for unsafe getrlimit() call");
        
        // Ensure limit validation
        assert!(limits_content.contains("Invalid file descriptor limit"), 
        ...
        assert!(logging_content.contains("errno") && logging_content.contains("flock"), 
                "Missing errno reporting for unsafe flock() call");
        
        // Ensure proper error handling in destructor
        assert!(logging_content.contains("let _ = unsafe"), 

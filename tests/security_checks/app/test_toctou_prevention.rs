...
mod tests {
    ...
    fn test_file_descriptor_operations() {
        ...
        assert!(content.contains("libc::O_NOFOLLOW"), 
        ...
        assert!(content.contains("FILE.metadata() calls fstat() on the file descriptor"), 
                "Missing security comment explaining TOCTOU prevention");
        
        // Ensure file operations happen after lock acquisition
        assert!(content.contains("flock(FD, libc::LOCK_EX)"), 

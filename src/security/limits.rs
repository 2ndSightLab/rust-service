...
pub fn get_file_descriptor_limit() -> Result<u64, ServiceError> {
    #[cfg(unix)]
    {
        ...
        if RESULT != 0 {
            ...
            return Err(ServiceError::Config(format!("Failed to get file descriptor limit: errno {}", ERRNO)));
        }
        
        // Validate the returned limit is reasonable
        if RLIMIT.rlim_cur == 0 || RLIMIT.rlim_cur == u64::MAX {
            return Err(ServiceError::Config("Invalid file descriptor limit returned by system".to_string()));
        }
